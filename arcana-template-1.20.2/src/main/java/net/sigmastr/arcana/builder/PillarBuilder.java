...
public class PillarBuilder {
    ...
    private void buildLayer() {
        ...
        for (int y = 0; y < currentHeight; y++) {
            BlockPos blockPos = startPos.down(-1).up(y);
                ...
                for (int zOffset : positions) {
                    ...
                    if (!(Math.abs(xOffset) == 2 && Math.abs(zOffset) == 2)) {
                        if (world.isAir(blockPos.add(xOffset, 0, zOffset))) {
                            world.setBlockState(blockPos.add(xOffset, 0, zOffset), hitBlock.getDefaultState());
                        }
                    }
                }
            }
        ...
        if (world.isAir(startPos)) {
            world.setBlockState(startPos, hitBlock.getDefaultState());
