...
public class NetherWandItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity playerEntity, Hand hand) {
        ...
        for (MagicSpellScrollItem magicSpellScrollItem : getMagicSpellScrollItems()) {
            if (item.equals(magicSpellScrollItem)) {
                ...
                // Apply 50% reduction for "nether" spells
        ...
        if (playerEntity.getStackInHand(hand).getItem() == ArcanaItems.NETHER_WAND && playerEntity.totalExperience >= -cost && !cast) {
                    ...
                    switch (spell) {
                        ...
                        case "build_pillar" -> {
                            ...
                            if (blockHitResult.getType() == HitResult.Type.BLOCK) {
                                ...
                                if (NaturalBlocks.isNatural(hitBlock)) {
                                    PillarBuilder pillarBuilder = new PillarBuilder((ServerWorld) world, pos, direction, hitBlock, 10, ArcanaItems.NETHER_WAND);
                        ...
                        case "build_stairs" -> {
                            ...
                            if (blockHitResult.getType() == HitResult.Type.BLOCK) {
                                ...
                                Block hitBlock = hitBlockState.getBlock();
                                if (NaturalBlocks.isNatural(hitBlock)) {
                                    StairsBuilder stairsBuilder = new StairsBuilder((ServerWorld) world, pos, direction, hitBlock, 10, ArcanaItems.NETHER_WAND, playerEntity.isSneaking());
    ...
    public Text getName(ItemStack stack) {
        return Text.literal("Nether Wand").formatted(Formatting.AQUA);
    }

    public static void setCast(boolean cast) {
        NetherWandItem.cast = cast;
    }
